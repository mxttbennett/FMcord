...
exports.run = async (client, message, args) => {
  ...
  try {
	...
	var x = 1;
	var y = 1;
    var data = await lib.user.getTopAlbums(user, `7day`, 1, mult);
	if (rand == 1){
		while (data.topalbums.album[0] == null){
			mult = getRandomInt(1, 100);
			data = await lib.user.getTopAlbums(user, `7day`, 1, mult);
		}
	}
    ...
    await message.reply(`your #` + mult + ` album of the past week: **` + data.topalbums.album[0].artist.name + `** — ***` + data.topalbums.album[0].name + `*** with **` + data.topalbums.album[0].playcount + `** plays`, { file: buffer });
...
exports.help = {
  name: `aw`,
  description: `**A**LBUM IN **W**EEK: Shows the album you have for the specified ranking in the past week. If no number is provided, it chooses randomly from your top 100 albums of the week.`,
  usage: `aw <#>`
};
