...
exports.run = async (client, message, args) => {
  ...
  while(trial < 3){
	  try {
		...
		var date2 = parseInt(date - 7776000);
		...
		var avg = parseFloat((total/(7776000/86400)).toFixed(2));
		...
		var avg2 = parseFloat((sum/(7776000/86400)).toFixed(2));
		...
		const buffer = finalCanvas.toBuffer();
			...
			try{
				...
				.setAuthor(`3-Month Chart (Page #` + page.toString() + `)`, message.author.displayAvatarURL(), 'https://www.last.fm/user/'+name+`/library/albums?date_preset=LAST_90_DAYS&page=` + page.toString())
...
exports.help = {
  name: `3m`,
  description: `**3**-**M**ONTH CHART: fetches a 5x10 3-month chart`,
  usage: `3m <page #>`
};
