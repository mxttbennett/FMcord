...
exports.run = async (client, message, args) => {
  ...
  try {
	...
	var x = 1;
	var y = 1;
    var data = await lib.user.getTopAlbums(user, `12month`, 1, mult);
	if (rand == 1){
		while (data.topalbums.album[0] == null){
			mult = getRandomInt(1, 750);
			data = await lib.user.getTopAlbums(user, `12month`, 1, mult);
		}
	}
    ...
    await message.reply(`your #` + mult + ` album of the past year: **` + data.topalbums.album[0].artist.name + `** — ***` + data.topalbums.album[0].name + `*** with **` + data.topalbums.album[0].playcount + `** plays`, { file: buffer });
...
exports.help = {
  name: `ay`,
  description: `**A**LBUM IN **Y**EAR: Shows the album you have for the specified ranking in the past year. If no number is provided, it chooses randomly from your top 750 albums of the year.`,
  usage: `ay <#>`
};
