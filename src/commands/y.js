...
exports.run = async (client, message, args) => {
  ...
  while(trial < 3){
	  try {
		...
		var date2 = parseInt(date - 31556952);
		...
		var avg = parseFloat((total/(31556952/86400)).toFixed(2));
		...
		var avg2 = parseFloat((sum/(31556952/86400)).toFixed(2));
		...
		const buffer = finalCanvas.toBuffer();
			...
			try{
				...
				.setAuthor(`Yearly Chart (Page #` + page.toString() + `)`, message.author.displayAvatarURL(), 'https://www.last.fm/user/'+name+`/library/albums?date_preset=LAST_365_DAYS&page=` + page.toString())
