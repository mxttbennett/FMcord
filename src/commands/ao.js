...
exports.run = async (client, message, args) => {
  ...
  try {
	...
	var x = 1;
	var y = 1;
    var data = await lib.user.getTopAlbums(user, period, 1, mult);
	if (rand == 1){
		while (data.topalbums.album[0] == null){
			mult = getRandomInt(1, 1500);
			data = await lib.user.getTopAlbums(user, period, 1, mult);
		}
	}
    ...
    const buffer = finalCanvas.toBuffer();
     await message.reply(`your #` + mult + ` album of all-time: **` + data.topalbums.album[0].artist.name + `** — ***` + data.topalbums.album[0].name + `*** with **` + data.topalbums.album[0].playcount + `** plays`, { file: buffer });
...
exports.help = {
  name: `ao`,
  description: `**A**LBUM IN **O**VERALL: Shows the album you have for the specified ranking in your overall last.fm charts. If no number is provided, it chooses randomly from your top 1500 albums of all-time.`,
  usage: `ao <#>`
};
