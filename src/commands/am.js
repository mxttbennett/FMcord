...
exports.run = async (client, message, args) => {
  ...
  try {
	...
	var x = 1;
	var y = 1;
    var data = await lib.user.getTopAlbums(user, `1month`, 1, mult);
	if (rand == 1){
		while (data.topalbums.album[0] == null){
			mult = getRandomInt(1, 200);
			data = await lib.user.getTopAlbums(user, `1month`, 1, mult);
		}
	}
    ...
    const buffer = finalCanvas.toBuffer();
     await message.reply(`your #` + mult + ` album of the past month: **` + data.topalbums.album[0].artist.name + `** — ***` + data.topalbums.album[0].name + `*** with **` + data.topalbums.album[0].playcount + `** plays`, { file: buffer });
...
exports.help = {
  name: `am`,
  description: `**A**LBUM IN **M**ONTH: Shows the album you have for the specified ranking in the past month. If no number is provided, it chooses randomly from your top 200 albums of the month.`,
  usage: `am <#>`
};
