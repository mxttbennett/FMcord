...
exports.run = async (client, message, args) => {
  ...
  while(trial < 3){
	  try {
		...
		if(!page){
			page = 1;
			data = await lib.user.getTopAlbums(user, period, `30`, `1`);
		}
		else{
			if(parseInt(page) > 0){
			data = await lib.user.getTopAlbums(user, period, `30`, parseInt(page).toString());
			}
			else{
				page = 1;
				const data = await lib.user.getTopAlbums(user, period, `30`, `1`);
			}
		}
		...
		album.forEach(a => {
		  if (a.image[3][`#text`].length > 0) {
			 //console.log(a.image[3][`#text`] + ` ${++test}`);
		...
		var date2 = parseInt(date - 604800);
		...
		var avg2 = parseFloat((sum/7).toFixed(2));
		...
		const buffer = finalCanvas.toBuffer();
			...
			try{
				...
				.setAuthor(`Weekly Chart (Page #` + page.toString() + `)`, message.author.displayAvatarURL(), 'https://www.last.fm/user/'+name+`/library/albums?date_preset=LAST_7_DAYS&page=1`)
...
//var iter = 1;
//const thread = getRandomInt(1, 100000).toString() + ` w`;
const cType = `w`;
